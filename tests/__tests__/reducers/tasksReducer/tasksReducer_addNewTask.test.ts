import { addTask } from '../../../../src/store/actions/subtaskActions';
...
describe('tasks reducer - add new task', () => {
  test('add a new task(eng) in an empty list of tasks', () => {
    ...
    const listIndex = 0;
    const newState = tasksReducer(emptyTasksState, addTask(name, listIndex));
  ...
  test('add a new task(eng+rus+num) in a list which have another tasks', () => {
    ...
    const listIndex = 1;
    const newState = tasksReducer(nonEmptyTasksState, addTask(name, listIndex));

    expect(newState).toStrictEqual({
      tasks: [[], ['123', 'rty', 'йцу', 'rtg кена гп 123'], []],
    });
  });
});
